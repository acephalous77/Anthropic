...
- Break rule-of-three lists: keep the strongest item, or use two, or four. One tricolon per page maximum. This applies at paragraph scale too: the **triptych** — three parallel enumerated positions or mini-sections ("The first is… The second… The third…") — is the same tell writ large, and it survives cosmetic rewording because the tell is the three-beat *shape*. Treat triads as a **budgeted resource**: if the voice profile sets a ration (e.g., one per 20,000 words), count shapes against it by eye — staged triads span paragraphs and evade regexes — and spend the budget on triads carrying framework weight (a canonical three-part model earns a slot; rhetorical staging never does). Everything over budget dissolves into continuous argument, extends past three, or contracts to one point.
...
Constructions:
- **Negative parallelism — hunt this FIRST.** The signature current-generation tell, and the single highest-priority fix in this skill. Two sub-families with different treatment:
  - **The corrective register (always an error):** "It's not just X, it's Y" / "This isn't X. It's Y." / "X is not Y — it's Z" (any subject) / "not yet X. It is Y" / "not so much X as Y" / "less about X than Y" / "more than just X" / the trailing correction "Y, not X." / "No A. No B. Just C." — plus the forms regexes miss: predicate denial followed by the real predicate ("Hope is not the closing register; what stands in its place is…") and **negative-definition sequences** ("It is not a jeremiad… Nor is it… And it is not…" — rewrite as affirmative definitions: "The jeremiad is the first temptation…"). Corpus measurement shows this register can have a ZERO base rate in a human writer while machine-touched drafts run it at more than one per thousand words — treat it as a **provenance marker**: its presence flags the passage as machine-written. Rewrite as a direct affirmative claim, letting evidence carry the contrast. Only grammatical negation with no corrective parallel is exempt ("someone who is not oneself," "the book does not resolve it").
  - **The classical correlative (review, never add):** "not only X but also Y" / "not X, but Y." Real writers use these at a low rate. Keep ones the author would defend aloud; never introduce new ones; if the author's voice profile measures their base rate at zero, escalate to error.
