...
- **Negative parallelism — hunt this FIRST.** The signature current-generation tell, and the single highest-priority fix in this skill. Two sub-families with different treatment:
  - **The resumptive reveal (always an error):** "It's not just X, it's Y" / "This isn't X. It's Y." / "X is not Y — it's Z" (any subject) / "not so much X as Y" / "less about X than Y" / "more than just X" / the trailing correction "Y, not X." / "No A. No B. Just C." Corpus measurement shows this form can have a ZERO base rate in a human writer while machine-touched drafts run it at more than one per thousand words — treat it as a **provenance marker**: its presence flags the passage as machine-written. Rewrite as a direct affirmative claim, letting evidence carry the contrast.
